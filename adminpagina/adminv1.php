...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['action'])) {
        switch ($_POST['action']) {
            case 'add_printer':
                $printerData = [
                    'Naam' => $_POST['printer_naam'],
                    'Status' => $_POST['printer_status'],
                    'Info' => $_POST['printer_info']
                ];
                
                $response = callAPI('POST', '/printer_api.php', $printerData);
                
                if ($response['code'] === 201) {
                    $message = "Printer succesvol toegevoegd!";
                } else {
                    $error = "Fout bij toevoegen printer: " . $response['data']['message'];
                }
                break;
            ...
            case 'update_printer':
                ...
                $response = callAPI('PUT', "/printer_api.php?id=$printerId", $printerData);
                
                if ($response['code'] === 200) {
                    $message = "Printer succesvol bijgewerkt!";
                } else {
                    $error = "Fout bij bijwerken printer: " . $response['data']['message'];
                }
                break;
                
            case 'change_printer_status':
                $printerId = $_POST['printer_id'];
                $status = $_POST['status'];
                
                $response = callAPI('PUT', "/printer_api.php?id=$printerId", ['Status' => $status]);
                
                if ($response['code'] === 200) {
                    $message = "Printer status succesvol bijgewerkt!";
                } else {
                    $error = "Fout bij bijwerken printer status: " . $response['data']['message'];
                }
                break;
            ...
            case 'update_reservation':
                ...
                $reservationData = [
                    'Pr_Start' => $_POST['start_time'],
                    'Pr_End' => $_POST['end_time'],
                    'Comment' => $_POST['comment']
                ];
                
                $response = callAPI('PUT', "/reservatie_api.php?id=$reservationId", $reservationData);
                
                if ($response['code'] === 200) {
                    // Haal gebruiker informatie op om e-mail te sturen
                    $resResponse = callAPI('GET', "/reservatie_api.php?id=$reservationId", null);
                    
                    if ($resResponse['code'] === 200) {
                        $reservation = $resResponse['data']['data'];
                        $userId = $reservation['User_ID'];
                        
                        $userResponse = callAPI('GET', "/gebruiker_api.php?id=$userId", null);
                        
                        if ($userResponse['code'] === 200) {
                            $user = $userResponse['data']['data'];
                            $userEmail = $user['Email'];
                            $userName = $user['Voornaam'] . ' ' . $user['Naam'];
                            
                            $subject = "Wijziging in je MaakLab reservering";
                            $emailMessage = "Er is een wijziging aangebracht in je reservering (ID: $reservationId).<br>
                                           Nieuwe starttijd: " . $_POST['start_time'] . "<br>
                                           Nieuwe eindtijd: " . $_POST['end_time'] . "<br>";
                            
                            if (sendNotificationEmail($userEmail, $userName, $subject, $emailMessage)) {
                                $message = "Reservering bijgewerkt en e-mail verzonden naar gebruiker.";
                            } else {
                                $message = "Reservering bijgewerkt maar e-mail kon niet worden verzonden.";
                            }
                        } else {
                            $message = "Reservering bijgewerkt maar gebruikersinformatie kon niet worden opgehaald.";
                        }
                    } else {
                        $message = "Reservering bijgewerkt maar details konden niet worden opgehaald.";
                    }
                } else {
                    $error = "Fout bij bijwerken reservering: " . $response['data']['message'];
                }
                break;
            ...
            case 'update_user_role':
                ...
                $response = callAPI('PUT', "/gebruiker_api.php?id=$userId", ['rol' => $newRole]);
                ...
                if ($response['code'] === 200) {
                    ...
                    if ($userResponse['code'] === 200) {
                        ...
                        $subject = "Wijziging in je MaakLab rol";
                        $emailMessage = "Je rol is gewijzigd naar: $newRole.<br>
                                       Dit kan wijzigingen betekenen in je rechten binnen het MaakLab systeem.";
                        
                        if (sendNotificationEmail($userEmail, $userName, $subject, $emailMessage)) {
                            $message = "Gebruikersrol bijgewerkt en e-mail verzonden naar gebruiker.";
                        } else {
                            $message = "Gebruikersrol bijgewerkt maar e-mail kon niet worden verzonden.";
                        }
                    } else {
                        $message = "Gebruikersrol bijgewerkt maar gebruikersinformatie kon niet worden opgehaald.";
                    }
                } else {
                    $error = "Fout bij bijwerken gebruikersrol: " . $response['data']['message'];
                }
                break;
        }
    }
...
<body>
            ...
            <div class="modal-content">
                ...
                <form method="post" action="">
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            <label for="edit_reservation_comment" class="form-label"></label>
    ...
    <!-- JavaScript voor modals en andere functionaliteiten -->
    ...
    <!-- Toevoegen van extra UI elementen voor het beheren van printers via touchscreen -->
    ...
    <!-- Uitgebreid zoek- en filterformulier voor reserveringen -->
